...
## bandwidth range: 0~4
indep_vars <- c("price_avg", "coca_area", "seizures", "river_length", "road_length", "population", "airport", "ferry", "police", "military", "lab_reported", "lab_residual", "armed_group_overlap")
max_bwd <- 4
# setwd("/Users/R")
...
regression_data_CF_2013 <- regression_data_CF_2013 %>% left_join(violence_combined, by = "id") %>% 
  mutate(FARC = ifelse(is.na(FARC), 0, FARC),
         ...
         armed_group = ifelse(AUC == 1, 1, armed_group))
...
regression_data_CF_2013 <- regression_data_CF_2013 %>% mutate(armed_group_overlap = as.factor(armed_group + or(ELN, FARC))) %>% select(-armed_group, -FARC, -ELN, -AUC)
regression_data_CF_2014 <- regression_data_CF_2014 %>% mutate(armed_group_overlap = as.factor(armed_group + or(ELN, FARC))) %>% select(-armed_group, -FARC, -ELN, -AUC)
regression_data_CF_2016 <- regression_data_CF_2016 %>% mutate(armed_group_overlap = as.factor(armed_group + or(ELN, FARC))) %>% select(-armed_group, -FARC, -ELN, -AUC)
regression_data_CF_2017 <- regression_data_CF_2017 %>% mutate(armed_group_overlap = as.factor(armed_group + or(ELN, FARC))) %>% select(-armed_group, -FARC, -ELN, -AUC)
...
local_GWR_PML_CF <- function(type.measure_="default", AUC_mat, sig_level_=0.05, gwr_PML_data_, method_, n_drop, interact_=F, scale_11_=F, weight_=NULL, dep_var, price_) {
  ...
  for (i in 1:nrow(AUC_mat)) {
    ...
    for (j in 1:length(bwd_range)) {
      ...
      if (sort(neighbor_ij$armed_group_overlap %>% table)[1:2] %>% sum < n_drop | n_unique_vals[["armed_group_overlap"]] < 2) neighbor_ij$armed_group_overlap <- NULL
...
local_GWR_PML_1_year <- function(dep_var_, seed_model, reg_data_year, year_, price=F, weight_in=NULL) {
  ...
  write.csv(local_GWR_coefs_PML_list$AUC_mat,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s armed_group_overlap all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).csv",
                    dep_var_, year_, title_for_price), row.names = F)
  save("local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo",
       file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s armed_group_overlap all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).RData",
...
local_gwr_PML_coef_map_by_AUC <- function(local_GWR_coefs_list, PML_best_bw_tbl_, criteria, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  write.csv(coef_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s armed_group_overlap all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)

  write.csv(pval_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s armed_group_overlap all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  ...
  for (i in c(2, 4:length(coef_table))) {
    ...
    ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s (%i)/local GWR PML coef by AUC armed_group_overlap %s %s all var drop n_drop=%i %i data %s CF (%s).png",
...
local_gwr_PML_coef_map_by_AUC_year <- function(dep_var_, year_, price_=F) {
  ...
  indep_vars_in[length(indep_vars_in)] <- "armed_group_overlap1"
  indep_vars_in <- c("Intercept", indep_vars_in, "armed_group_overlap2")
  
  PML_AUC_score_var_drop_log_seizure_10_loo <-
    read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s armed_group_overlap all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).csv",
  ...
  load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s armed_group_overlap all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).RData",
               dep_var_, year_, title_for_price))
...
# prediction check
PML_gwr_coefs_AUC_CF_2013 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination armed_group_overlap all var drop by AUC n_drop=10 2013 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2014 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination armed_group_overlap all var drop by AUC n_drop=10 2014 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2016 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination armed_group_overlap all var drop by AUC n_drop=10 2016 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2017 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination armed_group_overlap all var drop by AUC n_drop=10 2017 data no price CF (04-13-2026).csv") %>% as_tibble
...
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year (armed_group_overlap)/roc curve GWR hyd destinations 2013.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year (armed_group_overlap)/roc curve GWR hyd destinations 2014.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year (armed_group_overlap)/roc curve GWR hyd destinations 2016.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year (armed_group_overlap)/roc curve GWR hyd destinations 2017.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year (armed_group_overlap)/roc curve GWR hyd destinations 2017 using 2016 coefs.png")
