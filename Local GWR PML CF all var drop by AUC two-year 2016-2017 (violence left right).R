...
local_GWR_PML_2_years <- function(dep_var_, seed_model, reg_data_year1, reg_data_year2, year_, price=F, weight_in=NULL) {
  ...
  write.csv(local_GWR_coefs_PML_list$AUC_mat,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop AUC n_drop=10 %s data %s CF (05-01-2026).csv",
                    dep_var_, year_, title_for_price), row.names = F)
  save("local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo",
       file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop by AUC n_drop=10 %s data %s CF (05-01-2026).RData",
  ...
  # write.csv(local_GWR_coefs_PML_list$AUC_mat,
  #           sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight %i-%i %s data CF (05-01-2026).csv",
  ...
  #      file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight %i-%i %s data CF (05-01-2026).RData",
...
local_gwr_PML_coef_map_by_AUC <- function(local_GWR_coefs_list, PML_best_bw_tbl_, criteria, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  write.csv(coef_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s violence_all left-right all var drop by AUC n_drop=%i %s data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  
  write.csv(pval_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s violence_all left-right all var drop by AUC n_drop=%i %s data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  ...
  for (i in c(2, 4:length(coef_table))) {
    ...
    ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s (%s)/local GWR coef by AUC violence_all left-right %s %s all var drop %i %s data %s CF (%s).png",
...
local_gwr_PML_coef_map_by_AUC_year <- function(dep_var_, year_, price_=F) {
  ...
  PML_AUC_score_var_drop_log_seizure_10_loo <-
    read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop AUC n_drop=10 %s data %s CF (05-01-2026).csv",
                     dep_var_,year_, title_for_price)) %>% as_tibble
  # PML_AUC_score_var_drop_log_seizure_10_loo <-
  #   read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight 7-3 %s data CF %s (05-01-2026).csv",
  #                    dep_var_, year_, title_for_price)) %>% as_tibble
  
  # local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo
  load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop by AUC n_drop=10 %s data %s CF (05-01-2026).RData",
               dep_var_, year_, title_for_price))
  # local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo_7_3
  # load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight 7-3 %s data %s CF (05-01-2026).RData",
  #              dep_var_, year_, title_for_price))
...
# prediction check
PML_gwr_coefs_AUC_CF_2013_2014 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right all var drop by AUC n_drop=10 2013-2014 data no price CF (05-01-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2016_2017 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right all var drop by AUC n_drop=10 2016-2017 data no price CF (05-01-2026).csv") %>% as_tibble
...
GWR_predict_year_CF <- function(PML_gwr_coefs, reg_data_year, dep_var, no_price, threshold=0.5) {
  indep_vars <- c("price_avg", "coca_area", "seizures", "river_length", "road_length", "population", "airport", "ferry", "police", "military", "lab_reported", "lab_residual", "left_wing", "right_paramilitary", "left.right")
...
GWR_predict_2016_CF_with_1617_coef <- GWR_predict_year_CF(PML_gwr_coefs_AUC_CF_2016_2017, regression_data_CF_2016 %>% mutate(left.right = left_wing*right_paramilitary), "hyd_destination", no_price=T)
GWR_predict_2017_CF_with_1617_coef <- GWR_predict_year_CF(PML_gwr_coefs_AUC_CF_2016_2017, regression_data_CF_2017 %>% mutate(left.right = left_wing*right_paramilitary), "hyd_destination", no_price=T)
...
# png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PMLhyd destinations 2013 with 1314 coef.png")
...
# dev.off()
# png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PMLhyd destinations 2014 with 1314 coef.png")
...
# dev.off()
# png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PMLhyd destinations 2016 with 1314 coef.png")
...
# dev.off()
# png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PMLhyd destinations 2017 with 1314 coef.png")
...
# dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PMLhyd destinations 2016 with 1617 coef.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PMLhyd destinations 2017 with 1617 coef.png")
...
png("Colombia Data/local GWR PML result predicted prices/roc curves CF by two-year/roc curve GWR PMLpredictions of hyd destinations in 2014 with 2013.png")
...
png("Colombia Data/local GWR PML result predicted prices/roc curves CF by two-year/roc curve GWR PMLpredictions of hyd destinations in 2017 with 2016.png")
...
global_reg_two_year_CF <- function(reg_data_year1, reg_data_year2, dep_var, no_price) {
  indep_vars <- c("price_avg", "coca_area", "seizures", "river_length", "road_length", "population", "airport", "ferry", "police", "military", "lab_reported", "lab_residual", "left_wing", "right_paramilitary")
  dep_var_index <- which(names(reg_data_year1) == dep_var)
  names(reg_data_year1)[dep_var_index] <- "y"
  names(reg_data_year2)[dep_var_index] <- "y"
  ...
  reg_data_year_pred <- bind_rows(reg_data_year1, reg_data_year2)
...
global_reg_coefs_CF_1617 <- global_reg_two_year_CF(regression_data_CF_2016, regression_data_CF_2017, "hyd_destination", no_price=T)

global_predict_2016_CF_with_1617_coef <- tibble(id = regression_data_CF_2016$id, y = global_reg_coefs_CF_1617$y[1:1120], pi_hat = global_reg_coefs_CF_1617$fitted.values[1:1120])
global_predict_2017_CF_with_1617_coef <- tibble(id = regression_data_CF_2017$id, y = global_reg_coefs_CF_1617$y[1121:2240], pi_hat = global_reg_coefs_CF_1617$fitted.values[1121:2240])
comparison_with_GWR_2016 <- left_join(GWR_predict_2016_CF_with_1617_coef, global_predict_2016_CF_with_1617_coef %>% rename(pi_hat_global = pi_hat), by="id")
comparison_with_GWR_2017 <- left_join(GWR_predict_2017_CF_with_1617_coef, global_predict_2017_CF_with_1617_coef %>% rename(pi_hat_global = pi_hat), by="id")

global_ROC_2016_CF_with_1617_coef <- ROC_pred(global_predict_2016_CF_with_1617_coef)
global_ROC_2017_CF_with_1617_coef <- ROC_pred(global_predict_2017_CF_with_1617_coef)

png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve global hyd destinations 2016 with 1617 coef.png")
plot(global_ROC_2016_CF_with_1617_coef, main="hyd destination - predictions 2016"); text(0.1, 0, paste("AUC:", round(global_ROC_2016_CF_with_1617_coef$auc, 2)))
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve global hyd destinations 2017 with 1617 coef.png")
plot(global_ROC_2017_CF_with_1617_coef, main="hyd destination - predictions 2017"); text(0.1, 0, paste("AUC:", round(global_ROC_2017_CF_with_1617_coef$auc, 2)))
dev.off()
