...
local_gwr_PML_coef_map_by_F1_common_range <- function(min_max_table, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  for (year_ in years) {
    coef_table <- read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s leave-one-out PML_log_seizure_coca_bw_F1 all var drop %i %i data %s CF selected rivers (%s).csv",
                                   dep_var, n_drop, year_, title_for_price, date_)) %>% as_tibble
    pval_table <- read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s leave-one-out PML_log_seizure_coca_bw_F1 all var drop %i %i data %s CF selected rivers (%s).csv",
                                   dep_var, n_drop, year_, title_for_price, date_)) %>% as_tibble
    ...
    for (i in 4:length(coef_table)) {
      ...
      ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps CF by year ()/%s common range (%i)/local GWR PML coef by drop %s %s all var drop %i %i data %s CF (%s).png",
                     dep_var, year_, var_name, dep_var, n_drop, year_, title_for_price, date_),
             gwr_coef_map, scale=1)
      
      # ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s weight 7-3 selected rivers common range (%i)/local GWR PML coef by drop %s %s all var drop %i weight 7-3 %i data %s CF (%s).png",
      #                dep_var, year_, var_name, dep_var, n_drop, year_, title_for_price, date_),
      #        gwr_coef_map, scale=1)
    }
  }
