...
## bandwidth range: 0~4
indep_vars <- c("price_avg", "coca_area", "seizures", "river_length", "road_length", "population", "airport", "ferry", "police", "military", "lab_reported", "lab_residual", "left_n_violence", "right_n_violence")
max_bwd <- 4
# setwd("/Users/R")
...
########## used bandwidth range 0.5~3.0 due to the time limit
{
  ...
  violence_combined <- violence_all %>%
    ...
    group_by(id) %>% # removed year under the assumption that paramilitary and guerrilla groups do not relocate that much
    summarize(Guerrilla = sum(Guerrilla),
              FARC = sum(FARC),
              ELN = sum(ELN),
              AUC = sum(AUC),
              Front = sum(Front),
              Bloque = sum(Bloque)) %>% ungroup %>% 
    # right_join(regression_data_CF_2016 %>% select(id, armed_group) %>% mutate(paramilitary = armed_group), by="id") %>% 
    mutate(left_n_violence = log(1 + rowSums(pick(Guerrilla:ELN, Front))),
           right_n_violence = log(1 + rowSums(pick(AUC, Bloque))))
  
  regression_data_CF_2013 <- regression_data_CF_2013 %>% select(-armed_group) %>% left_join(violence_combined %>% select(id, left_n_violence, right_n_violence), by = "id")
  regression_data_CF_2014 <- regression_data_CF_2014 %>% select(-armed_group) %>% left_join(violence_combined %>% select(id, left_n_violence, right_n_violence), by = "id")
  regression_data_CF_2016 <- regression_data_CF_2016 %>% select(-armed_group) %>% left_join(violence_combined %>% select(id, left_n_violence, right_n_violence), by = "id")
  regression_data_CF_2017 <- regression_data_CF_2017 %>% select(-armed_group) %>% left_join(violence_combined %>% select(id, left_n_violence, right_n_violence), by = "id")
  regression_data_CF_2016[is.na(regression_data_CF_2016)] <- 0
  regression_data_CF_2017[is.na(regression_data_CF_2017)] <- 0
...
local_GWR_PML_CF <- function(type.measure_="default", AUC_mat, sig_level_=0.05, gwr_PML_data_, method_, n_drop, interact_=F, scale_11_=F, weight_=NULL, dep_var, price_) {
  bwd_range <- seq(0.5, max_bwd, by=0.1)
  ...
  for (i in 1:nrow(AUC_mat)) {
    ...
    for (j in 1:length(bwd_range)) {
      ...
      if (n_unique_vals[["left_n_violence"]] < n_drop) {
        neighbor_ij$left_n_violence <- NULL
        n_unique_vals[["left_n_violence"]] <- n_drop
      }
      if (n_unique_vals[["right_n_violence"]] < n_drop) {
        neighbor_ij$right_n_violence <- NULL
        n_unique_vals[["right_n_violence"]] <- n_drop
      }
...
local_GWR_PML_1_year <- function(dep_var_, seed_model, reg_data_year, year_, price=F, weight_in=NULL) {
  ...
  write.csv(local_GWR_coefs_PML_list$AUC_mat,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).csv",
                    dep_var_, year_, title_for_price), row.names = F)
  save("local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo",
       file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).RData",
  ...
  # write.csv(local_GWR_coefs_PML_list$AUC_mat,
  #           sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight %i-%i %i data CF (04-13-2026).csv",
  ...
  #      file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight %i-%i %i data CF (04-13-2026).RData",
...
local_gwr_PML_coef_map_by_AUC <- function(local_GWR_coefs_list, PML_best_bw_tbl_, criteria, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  write.csv(coef_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s violence_all left-right log freq all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  
  write.csv(pval_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s violence_all left-right log freq all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  ...
  for (i in c(2, 4:length(coef_table))) {
    ...
    ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s (%i)/local GWR PML coef by AUC violence_all log freq %s %s all var drop n_drop=%i %i data %s CF (%s).png",
                   dep_var, year_, var_name, dep_var, n_drop, year_, title_for_price, date_),
           gwr_coef_map, scale=1)
    
    # ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s weight 7-3 (%i)/local GWR PML coef by drop %s %s %s all var %i weight 7-3 %i data %s CF (%s).png",
...
local_gwr_PML_coef_map_by_AUC_year <- function(dep_var_, year_, price_=F) {
  ...
  PML_AUC_score_var_drop_log_seizure_10_loo <-
    read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).csv",
                     dep_var_,year_, title_for_price)) %>% as_tibble
  # PML_AUC_score_var_drop_log_seizure_10_loo <-
  #   read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight 7-3 %i data CF %s (04-13-2026).csv",
  #                    dep_var_, year_, title_for_price)) %>% as_tibble
  
  # local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo
  load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).RData",
               dep_var_, year_, title_for_price))
  # local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo_7_3
  # load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight 7-3 %i data %s CF (04-13-2026).RData",
  #              dep_var_, year_, title_for_price))
  ...
  local_gwr_PML_coef_map_by_AUC(local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo, PML_best_bw_tbl_var_drop, criteria="PML_log_seizure_coca_bw_AUC", dep_var = dep_var_,
                                indep_vars_ = indep_vars_in, n_drop=10, date_="04-13-2026", year_=year_, price=price_)
...
n_violence_max <- max(c(regression_data_CF_2016$left_n_violence, regression_data_CF_2016$right_n_violence))
n_violence_min <- min(c(regression_data_CF_2016$left_n_violence, regression_data_CF_2016$right_n_violence))
left_n_violence_all_years_map <- left_join(map_df, regression_data_CF_2016 %>% select(id, left_n_violence), by="id") %>% ggplot() +
  geom_polygon(aes(x=long, y=lat, group=group, fill=left_n_violence),
               color = "black",
               linewidth = 0.1) +
  expand_limits(x = map_df$long, y = map_df$lat) +
  coord_quickmap() +
  scale_fill_viridis_c(limits = c(n_violence_min, n_violence_max)) +
  labs(fill="log (n_violence): left", x="", y="", title="2013-2017") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        axis.text = element_blank(),
        line = element_blank())
right_n_violence_all_years_map <- left_join(map_df, regression_data_CF_2016 %>% select(id, right_n_violence), by="id") %>% ggplot() +
  geom_polygon(aes(x=long, y=lat, group=group, fill=right_n_violence),
               color = "black",
               linewidth = 0.1) +
  expand_limits(x = map_df$long, y = map_df$lat) +
  coord_quickmap() +
  scale_fill_viridis_c(limits = c(n_violence_min, n_violence_max)) +
  labs(fill="log (n_violence): right, paramilitary", x="", y="", title="2013-2017") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        axis.text = element_blank(),
        line = element_blank())
ggsave("Colombia Data/local GWR PML result predicted prices/violence maps/left right/log n_violence left 2013-2017.png", left_n_violence_all_years_map, scale=1)
ggsave("Colombia Data/local GWR PML result predicted prices/violence maps/left right/log n_violence right 2013-2017.png", right_n_violence_all_years_map, scale=1)

# prediction check
PML_gwr_coefs_AUC_CF_2013 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right log freq all var drop by AUC n_drop=10 2013 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2014 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right log freq all var drop by AUC n_drop=10 2014 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2016 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right log freq all var drop by AUC n_drop=10 2016 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2017 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right log freq all var drop by AUC n_drop=10 2017 data no price CF (04-13-2026).csv") %>% as_tibble
...
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq (violence_all)/roc curve GWR hyd destinations 2013.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq (violence_all)/roc curve GWR hyd destinations 2014.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq (violence_all)/roc curve GWR hyd destinations 2016.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq (violence_all)/roc curve GWR hyd destinations 2017.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq (violence_all)/roc curve GWR hyd destinations 2017 using 2016 coefs.png")
...
influential_var <- function(GWR_coef, reg_data_year_in, dep_var, no_price) {
  indep_vars <- c("price_avg", "coca_area", "seizures", "river_length", "road_length", "population", "airport", "ferry", "police", "military", "armed_group", "lab_reported", "lab_residual", "left_n_violence", "right_n_violence")
