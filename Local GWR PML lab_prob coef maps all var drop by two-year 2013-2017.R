...
PML_gwr_coefs_F1_lab_prob_all_years <- bind_rows(PML_gwr_coefs_F1_lab_prob_2013_2014, PML_gwr_coefs_F1_lab_prob_2016_2017)
...
min_max_table_ <- tibble(
  ...
  mid_coef = c(5, 10, 3, 10, 20, 5, 5, 5, 5, 5, 50)
)
...
local_gwr_PML_coef_map_by_F1_common_range <- function(min_max_table, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  for (year_ in years) {
    coef_table <- read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s leave-one-out PML_log_seizure_coca_bw_F1 all var drop %i %s data %s lab_prob (%s).csv",
                                   dep_var, n_drop, year_, title_for_price, date_)) %>% as_tibble
    pval_table <- read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s leave-one-out PML_log_seizure_coca_bw_F1 all var drop %i %s data %s lab_prob (%s).csv",
                                   dep_var, n_drop, year_, title_for_price, date_)) %>% as_tibble
    ...
    for (i in 4:length(coef_table)) {
      ...
      ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s common range (%s)/local GWR PML coef by drop %s %s all var drop %i %s data %s lab_prob (%s).png",
