...
regression_data_CF_1617 %>% filter(hyd_destination == 1) %>% nrow # 384
...
local_GWR_PML_lab_residual <- function(type.measure_="default", AUC_mat, sig_level_=0.05, gwr_PML_data_, method_, n_drop, interact_=F, scale_11_=F, weight_=NULL, dep_var, price_) {
  bwd_range <- seq(0.5, max_bwd, by=0.1)
...
local_GWR_PML_1_year <- function(dep_var_, seed_model, reg_data_year, year_, n_drop_=10, price=F, weight_in=NULL) {
  ...
  if (grepl("hyd", dep_var_)) {
    reg_data_year <- reg_data_year %>% 
      ...
      rename(price_avg=hyd_avg, lab_residual=hyd_lab_res, seizures=hyd_seizures) %>% 
      select(id, y, all_of(indep_vars))
  }else{
    reg_data_year <- reg_data_year %>% 
      select(-hyd_lab, -hyd_lab_res, -hyd_avg, -hyd_seizures) %>%
      rename(price_avg=base_avg, lab_residual=PPI_lab_res, seizures=base_seizures) %>% 
      select(id, y, all_of(indep_vars))
  }
  ...
  set.seed(seed_model)
  local_GWR_coefs_PML_list <- local_GWR_PML_lab_residual(dep_var = dep_var_, AUC_mat=AUC_mat_, gwr_PML_data_ = gwr_data, method_="var drop", sig_level_ = 0.1, n_drop = n_drop_, weight_ = weight_in, price_=price)
  ...
  write.csv(local_GWR_coefs_PML_list$AUC_mat,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop by AUC n_drop=%i %i data %s lab_residual (07-15-2026).csv",
                    dep_var_, n_drop_, year_, title_for_price), row.names = F)
  save("local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo",
       file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop by AUC n_drop=%i %i data %s lab_residual (07-15-2026).RData",
  ...
  # write.csv(local_GWR_coefs_PML_list$AUC_mat,
  #           sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight %i-%i %i data lab_residual (07-15-2026).csv",
  ...
  #      file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight %i-%i %i data lab_residual (07-15-2026).RData",
...
local_GWR_PML_1_year("hyd_destination", 435653, regression_data_CF_1617, 1617, n_drop_ = 10)
...
local_gwr_PML_coef_map_by_AUC <- function(local_GWR_coefs_list, PML_best_bw_tbl_, criteria, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  write.csv(coef_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s violence_all left-right all var drop by AUC n_drop=%i %i data %s lab_residual (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  
  write.csv(pval_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s violence_all left-right all var drop by AUC n_drop=%i %i data %s lab_residual (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  
  # for weighted reg
  # write.csv(coef_table,
  #           sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s leave-one-out %s all var drop %i weight 7-3 %i data %s lab_residual (%s).csv", dep_var, criteria, n_drop, year_, title_for_price, date_),
  #           row.names = F)
  # write.csv(pval_table,
  #           sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s leave-one-out %s all var drop %i weight 7-3 %i data %s lab_residual (%s).csv", dep_var, criteria, n_drop, year_, title_for_price, date_),
  #           row.names = F)
  ...
  for (i in c(2, 4:length(coef_table))) {
    ...
    ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s (%i)/local GWR PML coef by AUC violence_all left-right %s %s all var drop n_drop=%i %i data %s lab_residual.png",
                   dep_var, year_, var_name, dep_var, n_drop, year_, title_for_price),
           gwr_coef_map, scale=1)
    
    # ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s weight 7-3 (%i)/local GWR PML coef by drop %s %s %s all var %i weight 7-3 %i data %s lab_residual.png (%s).png",
...
local_gwr_PML_coef_map_by_AUC_year <- function(dep_var_, year_, n_drop_=10, price_=F) {
  ...
  PML_AUC_score_var_drop_log_seizure_10_loo <-
    read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop by AUC n_drop=%i %i data %s lab_residual (07-15-2026).csv",
                     dep_var_, n_drop_, year_, title_for_price)) %>% as_tibble
  # PML_AUC_score_var_drop_log_seizure_10_loo <-
  #   read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight 7-3 %i data %s lab_residual (07-15-2026).csv",
  #                    dep_var_, year_, title_for_price)) %>% as_tibble
  
  # local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo
  load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence_all left-right all var drop by AUC n_drop=%i %i data %s lab_residual (07-15-2026).RData",
               dep_var_, n_drop_, year_, title_for_price))
  # local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo_7_3
  # load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight 7-3 %i data %s lab_residual (07-15-2026).RData",
...
violence_all_years <- regression_data_CF_2016 %>% select(id, left_wing, right_paramilitary) %>% 
  mutate(armed_group = ifelse(left_wing == 1, "left-wing",
...
# prediction check
PML_gwr_coefs_AUC_CF_2013 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence_all left-right all var drop by AUC n_drop=10 2013 data no price CF (04-13-2026).csv") %>% as_tibble
...
GWR_predict_2013_CF_with_2013_coef <- GWR_predict_year_CF(PML_gwr_coefs_AUC_CF_2013, regression_data_CF_2013, "hyd_destination", no_price=T)
GWR_predict_2014_CF_with_2014_coef <- GWR_predict_year_CF(PML_gwr_coefs_AUC_CF_2014, regression_data_CF_2014, "hyd_destination", no_price=T)
GWR_predict_2016_CF_with_2016_coef <- GWR_predict_year_CF(PML_gwr_coefs_AUC_CF_2016, regression_data_CF_2016, "hyd_destination", no_price=T)
...
GWR_predict_2013_CF_with_2013_coef %>% filter(!is.na(pi_hat)) # 1099
GWR_predict_2014_CF_with_2014_coef %>% filter(!is.na(pi_hat)) # 1080
GWR_predict_2016_CF_with_2016_coef %>% filter(!is.na(pi_hat)) # 1096
GWR_predict_2017_CF_with_2017_coef %>% filter(!is.na(pi_hat)) # 1102
...
ROC_2017_CF_with_2016_coef <- ROC_pred(GWR_predict_2017_CF_with_2016_coef)
ROC_1617_CF_with_1617_coef <- ROC_pred(GWR_predict_1617_CF_with_1617_coef)

png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve GWR PML hyd destinations 2016-2017 combined.png")
...
ROC_2014_CF_with_2013_coef <- ROC_pred(GWR_predict_2014_CF_with_2013_coef)
ROC_2016_CF_with_2014_coef <- ROC_pred(GWR_predict_2016_CF_with_2014_coef)
...
plot(ROC_2017_CF_with_2016_coef)

plot(ROC_2014_CF_with_2013_coef)
plot(ROC_2016_CF_with_2014_coef)

png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right (violence_all)/roc curve GWR hyd destinations 2013.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right (violence_all)/roc curve GWR hyd destinations 2014.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right (violence_all)/roc curve GWR hyd destinations 2016.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right (violence_all)/roc curve GWR hyd destinations 2017.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right (violence_all)/roc curve GWR hyd destinations 2017 using 2016 coefs.png")
plot(ROC_2017_CF_with_2016_coef, main="hyd destination - predictions 2017 (2016 coefs)"); text(0.1, 0, paste("AUC:", round(ROC_2017_CF_with_2016_coef$auc, 2)))
dev.off()

GWR_predict_2013_CF_with_2013_coef %>% mutate(y = as.factor(y)) %>% 
  ggplot(aes(x=pi_hat, y=y)) + geom_point() + labs(title="predicted probabilities for y=1 cases 2013")
GWR_predict_2014_CF_with_2014_coef %>% mutate(y = as.factor(y)) %>% 
  ggplot(aes(x=pi_hat, y=y)) + geom_point() + labs(title="predicted probabilities for y=1 cases 2014")
GWR_predict_2016_CF_with_2016_coef %>% mutate(y = as.factor(y)) %>% 
  ggplot(aes(x=pi_hat, y=y)) + geom_point() + labs(title="predicted probabilities for y=1 cases 2016")
GWR_predict_2017_CF_with_2017_coef %>% mutate(y = as.factor(y)) %>% 
...
global_reg_coefs_CF_1617 <- global_reg_year_CF(regression_data_CF_1617, "hyd_destination", no_price=T)

global_predict_1617_CF_with_1617_coef <- tibble(id = regression_data_CF_1617$id, y = global_reg_coefs_CF_1617$y, pi_hat = global_reg_coefs_CF_1617$fitted.values)

global_ROC_1617_CF_with_1617_coef <- ROC_pred(global_predict_1617_CF_with_1617_coef)

png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by two-year (violence_all left-right)/roc curve global hyd destinations 2016-2017 combined.png")
plot(global_ROC_1617_CF_with_1617_coef, main="hyd destination - predictions 2016-2017 combined"); text(0.1, 0, paste("AUC:", round(global_ROC_1617_CF_with_1617_coef$auc, 2)))
dev.off()
...
variable_influence_CF_2013 <- influential_var(PML_gwr_coefs_AUC_CF_2013, regression_data_CF_2013, "hyd_destination", price=T)
variable_influence_CF_2014 <- influential_var(PML_gwr_coefs_AUC_CF_2014, regression_data_CF_2014, "hyd_destination", price=T)
variable_influence_CF_2016 <- influential_var(PML_gwr_coefs_AUC_CF_2016, regression_data_CF_2016, "hyd_destination", price=T)
variable_influence_CF_2017 <- influential_var(PML_gwr_coefs_AUC_CF_2017, regression_data_CF_2017, "hyd_destination", price=T)
variable_influence_CF_1617 <- influential_var(PML_gwr_coefs_AUC_CF_1617, regression_data_CF_1617, "hyd_destination", price=T)

variable_influence_map_CF <- function(variable_influence_CF_year, selected_id=NULL, dep_var, n_drop, year_) {
  ...
  for (i in c(2:length(variable_influence_CF_year))) {
    ...
    if (is.null(selected_id)) {
      ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/VI maps CF/%s (%i)/local GWR PML variable influence %s %s n_drop %i %i data CF.png",
                     dep_var, year_, var_name, dep_var, n_drop, year_),
             gwr_VI_map, scale=1)
    }else{
      ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/VI maps CF/VI maps CF (y=1)/%s (%i)/local GWR PML variable influence %s %s all var drop %i %i data %s lab_residual.png",
                     dep_var, year_, var_name, dep_var, n_drop, year_, title_for_price),
             gwr_VI_map, scale=1)
    }
  }
...
variable_influence_map_CF(variable_influence_CF_1617,
                          dep_var = "hyd_destination",
                          n_drop = 10,
                          year_ = 1617)

variable_influence_CF_1617_y <- variable_influence_CF_1617 %>% left_join(regression_data_CF_1617 %>% select(id, hyd_destination) %>% rename(y = hyd_destination), by = "id") %>% relocate(id, y)
variable_influence_CF_1617_y %>% apply(1,
                                       function(x) ifelse(x[2] == 1,
                                                          names(variable_influence_CF_1617 %>% select(-id, -Intercept))[which.max(x[4:17])],
                                                          names(variable_influence_CF_1617 %>% select(-id, -Intercept))[which.min(x[4:17])]))
...
PML_gwr_coefs_AUC_CF_2013 %>% filter(id %in% (GWR_predict_2013_CF_with_2013_coef %>% filter(y == 1 & pi_hat > 0.9) %>% pull(id))) %>% arrange(id) %>% print(n=100)
PML_gwr_coefs_AUC_CF_2013 %>% filter(id %in% (GWR_predict_2013_CF_with_2013_coef %>% filter(y == 1 & pi_hat < 0.1) %>% pull(id))) %>% arrange(id) %>% print(n=100)
...
PML_gwr_coefs_AUC_CF_2014 %>% filter(id %in% (GWR_predict_2014_CF_with_2014_coef %>% filter(y == 1 & pi_hat < 0.458) %>% pull(id)))
