...
PML_gwr_coefs_F1_CF_all_years <- bind_rows(PML_gwr_coefs_F1_CF_2013, PML_gwr_coefs_F1_CF_2014, PML_gwr_coefs_F1_CF_2016, PML_gwr_coefs_F1_CF_2017)
...
min_max_table_ <- tibble(
  ...
  mid_coef = c(5, 10, 10, 10, 30, 10, 5, 18, 10, 20, 10, 20)
)
...
local_gwr_PML_coef_map_by_F1_common_range <- function(min_max_table, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  # dep_var="hyd_destination"; alpha=0.1; n_drop=10; date_="02-04-2026"; indep_vars_=indep_vars_in; price=F
  years <- c(2013, 2014, 2016, 2017)
  title_for_price <- ifelse(price, "with price", "no price")
  
  for (year_ in years) {
    coef_table <- read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s leave-one-out PML_log_seizure_coca_bw_F1 all var drop %i %i data %s CF (%s).csv",
                                   dep_var, n_drop, year_, title_for_price, date_)) %>% as_tibble
    pval_table <- read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s leave-one-out PML_log_seizure_coca_bw_F1 all var drop %i %i data %s CF (%s).csv",
                                   dep_var, n_drop, year_, title_for_price, date_)) %>% as_tibble
    ...
    for (i in 4:length(coef_table)) {
      ...
      ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps CF by year/%s common range (%i)/local GWR PML coef by drop %s %s all var drop %i %i data %s CF common range (%s).png",
