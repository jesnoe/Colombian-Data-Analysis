...
########## used bandwidth range 0.5~3.0 due to the time limit
{
  ...
  violence_combined <- violence_all %>%
    ...
    mutate(left_n_violence = rowSums(pick(Guerrilla:ELN, Front)),
           right_n_violence = rowSums(pick(AUC, Bloque)))
  
  regression_data_CF_2013 <- regression_data_CF_2013 %>% select(-armed_group) %>% 
    left_join(violence_combined %>% filter(year == 2013) %>% select(id, left_n_violence, right_n_violence) %>%
                mutate(left_n_violence = log(1+left_n_violence),
                       right_n_violence = log(1 + right_n_violence)),
              by = "id")
  regression_data_CF_2014 <- regression_data_CF_2014 %>% select(-armed_group) %>% 
    left_join(violence_combined %>% filter(year == 2014) %>% select(id, left_n_violence, right_n_violence) %>%
                mutate(left_n_violence = log(1+left_n_violence),
                       right_n_violence = log(1 + right_n_violence)),
              by = "id")
  regression_data_CF_2016 <- regression_data_CF_2016 %>% select(-armed_group) %>%
    left_join(violence_combined %>% filter(year == 2016) %>% select(id, left_n_violence, right_n_violence) %>%
                mutate(left_n_violence = log(1+left_n_violence),
                       right_n_violence = log(1 + right_n_violence)),
              by = "id")
  regression_data_CF_2017 <- regression_data_CF_2017 %>% select(-armed_group) %>%
    left_join(violence_combined %>% filter(year == 2017) %>% select(id, left_n_violence, right_n_violence) %>%
                mutate(left_n_violence = log(1+left_n_violence),
                       right_n_violence = log(1 + right_n_violence)),
              by = "id")
...
local_GWR_PML_1_year <- function(dep_var_, seed_model, reg_data_year, year_, price=F, weight_in=NULL) {
  ...
  write.csv(local_GWR_coefs_PML_list$AUC_mat,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence each year left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).csv",
                    dep_var_, year_, title_for_price), row.names = F)
  save("local_GWR_coefs_PML_var_drop_log_seizure_scaled_loo",
       file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence each year left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).RData",
...
local_gwr_PML_coef_map_by_AUC <- function(local_GWR_coefs_list, PML_best_bw_tbl_, criteria, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  write.csv(coef_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s violence each year left-right log freq all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  
  write.csv(pval_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s violence each year left-right log freq all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  ...
  for (i in c(2, 4:length(coef_table))) {
    ...
    ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s (%i)/local GWR PML coef by AUC violence each year log freq %s %s all var drop n_drop=%i %i data %s CF (%s).png",
...
local_gwr_PML_coef_map_by_AUC_year <- function(dep_var_, year_, price_=F) {
  ...
  PML_AUC_score_var_drop_log_seizure_10_loo <-
    read.csv(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence each year left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).csv",
  ...
  load(sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s violence each year left-right log freq all var drop by AUC n_drop=10 %i data %s CF (04-13-2026).RData",
               dep_var_, year_, title_for_price))
...
# prediction check
PML_gwr_coefs_AUC_CF_2013 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence each year left-right log freq all var drop by AUC n_drop=10 2013 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2014 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence each year left-right log freq all var drop by AUC n_drop=10 2014 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2016 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence each year left-right log freq all var drop by AUC n_drop=10 2016 data no price CF (04-13-2026).csv") %>% as_tibble
PML_gwr_coefs_AUC_CF_2017 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination violence each year left-right log freq all var drop by AUC n_drop=10 2017 data no price CF (04-13-2026).csv") %>% as_tibble
...
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq each year (violence_all)/roc curve GWR hyd destinations 2013.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq each year (violence_all)/roc curve GWR hyd destinations 2014.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq each year (violence_all)/roc curve GWR hyd destinations 2016.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq each year (violence_all)/roc curve GWR hyd destinations 2017.png")
...
dev.off()
png("Colombia Data/local GWR PML result predicted prices/roc curves/roc curves CF by year left-right log freq each year (violence_all)/roc curve GWR hyd destinations 2017 using 2016 coefs.png")
