...
# }
# write.csv(n_reg_data_mat, "Colombia Data/local GWR number of neighbors (08-08-2024).csv", row.names=F)
n_reg_data_mat <- read.csv("Colombia Data/local GWR number of neighbors (08-08-2024).csv") %>% as_tibble
...
#     }
#     
#     neighbor_index <- which(local_gwr_dist[i,] <= bw_j)
#     neighbor_dist <- local_gwr_dist[i,neighbor_index]
...
# write.csv(aic_score_mat, "Colombia Data/local GWR AIC (08-08-2024).csv", row.names=F)
# save(list = c("gwr_result_list", "model_coef_list"), file = "Colombia Data/local GWR result (08-08-2024).RData")
aic_score_mat <- read.csv("Colombia Data/local GWR AIC (08-08-2024).csv") %>% as_tibble
load("Colombia Data/local GWR result (08-08-2024).RData")
