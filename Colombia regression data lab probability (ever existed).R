...
create_reg_data_combined_year <- function(anecdotal_annual, labs_reg_data, combined_year, is_CF) {
  ...
  labs_2steps_year <- labs_2steps_year %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>% 
    mutate(coca_area = scale(log(1+coca_area))[,1],
           base_avg=scale(base_avg)[,1],
           hyd_avg=scale(hyd_avg)[,1],
           population=scale(log(1+population))[,1],
           base_seizures = scale(log(1+base_seizures))[,1],
           hyd_seizures = scale(log(1+hyd_seizures))[,1])
...
labs_reg_data_2015 <- labs_reg_data %>% filter(year %in% c(2015)) %>% group_by(id) %>% 
  summarize(across(c(MUNICIPIO, DEPARTAMENTO, paste_avg:n_roads, erad_aerial, erad_manual), first),
            year = 1314,
            across(c(coca_area, n_PPI_labs, n_hyd_labs, n_armed_groups, coca_seizures:hyd_seizures), \(x) sum(x, na.rm = TRUE))) %>% ungroup
...
labs_reg_data_1617 <- labs_reg_data %>% filter(year %in% c(2016, 2017)) %>% group_by(id) %>% 
  summarize(across(c(MUNICIPIO, DEPARTAMENTO, paste_avg:n_roads, erad_aerial, erad_manual), first),
            year = 1617,
            across(c(coca_area, n_PPI_labs, n_hyd_labs, n_armed_groups, coca_seizures:hyd_seizures), \(x) sum(x, na.rm = TRUE))) %>% ungroup
labs_reg_data_1617$hyd_seizures <- labs_reg_data_2015$hyd_seizures
labs_reg_data_1617$base_seizures <- labs_reg_data_2015$base_seizures
labs_reg_data_1617$coca_seizures <- labs_reg_data_2015$coca_seizures
...
create_reg_data_combined_year_x_norm <- function(anecdotal_annual, labs_reg_data, combined_year, is_CF) {
  ...
  labs_2steps_year <- labs_2steps_year %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>% 
    mutate(coca_area = log(1+coca_area),
           base_avg=base_avg,
           hyd_avg=hyd_avg,
           population=log(1+population),
           base_seizures = log(1+base_seizures),
           hyd_seizures = log(1+hyd_seizures))
  
  labs_2steps_year_reg <- labs_2steps_year %>% 
    left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
    mutate(river_length = river_length / area_km2,
...
price_annual <- price %>% 
  ...
  filter(!is.na(id))
price_annual
price_annual[price_annual$year < 2017,-(1:2)] %>% apply(2, function(x) sum(is.na(x)))
...
## regression data
{
  ...
  cultivation_reg_data <- cultivation_coords %>% 
    relocate(id, MUNICIPIO, DEPARTAMENTO) %>% 
    pivot_longer(X1999:X2023, names_to="year", values_to="coca_area") %>% 
    mutate(year=substr(year,2,5) %>% as.integer)
...
### reg data for more recent years
{
  airports <- read.csv("Colombia Data/airports.csv") %>% as_tibble
  ferry <- read.csv("Colombia Data/ferry terminals.csv") %>% as_tibble
  police <- read.csv("Colombia Data/polices.csv") %>% as_tibble
...
# write.csv(regression_data_2016_logit, "Colombia Data/regression data all municipios lab_prob 2016 logit.csv", row.names = F)
{
  ...
  labs_2steps_2017 <- data_year %>% select(id:hyd_seizures, armed_group, population, PPI_lab, hyd_lab, base_source:hyd_destination) %>% left_join(airports, by="id") %>% relocate(id, year, base_source:hyd_destination)
}

# labs_2steps_2017 %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>%
#   left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
#   mutate(population=scale(population)[,1],
#          river_length = scale(river_length / area_km2)[,1],
#          road_length = scale(road_length / area_km2)[,1]) %>% 
#   select(-area_km2) %>% write.csv("Colombia Data/regression data all municipios raw prices and seizures 2017.csv", row.names = F)


labs_2steps_2017 <- labs_2steps_2017 %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>% 
  mutate(coca_area = scale(log(1+coca_area))[,1],
         base_avg=scale(base_avg)[,1],
         hyd_avg=scale(hyd_avg)[,1],
         population=scale(population)[,1],
...
labs_2steps_2017_reg <- labs_2steps_2017 %>% 
  left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
  mutate(river_length = scale(river_length / area_km2)[,1],
         road_length = scale(road_length / area_km2)[,1]) %>% 
  select(-area_km2)
...

{
  ...
  labs_2steps_2018 <- data_year %>% select(id:hyd_seizures, armed_group, population, PPI_lab, hyd_lab) %>% left_join(airports, by="id")
}

labs_2steps_2018 %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>%
  left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
  mutate(population=scale(population)[,1],
         river_length = scale(river_length / area_km2)[,1],
         road_length = scale(road_length / area_km2)[,1]) %>% 
  select(-area_km2) %>% 
  write.csv("Colombia Data/regression data all municipios raw prices and seizures 2018.csv", row.names = F)

## add probability that PPI/hyd labs exist
municipios_sf <- st_as_sf(municipios) %>% mutate(id = id %>% as.numeric) %>% filter(!(id %in% c(88001, 88564)))
municipios_sf$area_km2 <- st_area(municipios_sf) %>% units::set_units("km^2") %>% as.numeric

labs_2steps_2018 <- labs_2steps_2018 %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>% 
  mutate(coca_area = scale(log(1+coca_area))[,1],
         base_avg=scale(base_avg)[,1],
         hyd_avg=scale(hyd_avg)[,1],
         population=scale(population)[,1],
...
labs_2steps_2018_reg <- labs_2steps_2018 %>% 
  left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
  mutate(river_length = scale(river_length / area_km2)[,1],
         road_length = scale(road_length / area_km2)[,1]) %>% 
  select(-area_km2)
...
# prices are excluded to compute lab_prob due to small number of obs.
PPI_lab_glm_2018 <- glm(PPI_lab~., family="binomial", data=labs_2steps_2018_reg %>% select(-id, -base_avg, -hyd_avg, -hyd_seizures, hyd_lab))
summary(PPI_lab_glm_2018)

hyd_lab_glm_2018 <- glm(hyd_lab~., family="binomial", data=labs_2steps_2018_reg %>% select(-id, -base_avg, -hyd_avg, -base_seizures, PPI_lab))
summary(hyd_lab_glm_2018)

regression_data_2018 <- labs_2steps_2018_reg %>% 
  mutate(PPI_lab_prob=PPI_lab_glm_2018$fitted.values,
         hyd_lab_prob=hyd_lab_glm_2018$fitted.values) %>% 
  select(-PPI_lab, -hyd_lab) %>% relocate(id, PPI_lab_prob, hyd_lab_prob)

hyd_avg_lm <- lm(hyd_avg~., regression_data_2018 %>% select(-id, -base_avg, -base_seizures, -PPI_lab_prob))
summary(hyd_avg_lm)
hyd_avg_pred <- predict(hyd_avg_lm, regression_data_2018 %>% select(-id, -base_avg, -base_seizures, -PPI_lab_prob))
regression_data_2018$hyd_avg <- ifelse(is.na(regression_data_2018$hyd_avg), hyd_avg_pred, regression_data_2018$hyd_avg)

base_avg_lm <- lm(base_avg~., regression_data_2018 %>% select(-id, -hyd_avg, -hyd_seizures, -hyd_lab_prob))
summary(base_avg_lm)
base_avg_pred <- predict(base_avg_lm, regression_data_2018 %>% select(-id, -hyd_avg, -hyd_seizures, -hyd_lab_prob))
regression_data_2018$base_avg <- ifelse(is.na(regression_data_2018$base_avg), base_avg_pred, regression_data_2018$base_avg)

# write.csv(regression_data_2018,
#           "Colombia Data/regression data all municipios lab_prob 2018.csv", row.names = F)
...

{
  ...
  labs_2steps_2020 <- data_year %>% select(id:hyd_seizures, armed_group, population, PPI_lab, hyd_lab) %>% left_join(airports, by="id")
}

labs_2steps_2020 %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>%
  left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
  mutate(population=scale(population)[,1],
         river_length = scale(river_length / area_km2)[,1],
         road_length = scale(road_length / area_km2)[,1]) %>% 
...
labs_2steps_2020 <- labs_2steps_2020 %>% select(id, base_avg, hyd_avg, coca_area, base_seizures, hyd_seizures, PPI_lab, hyd_lab, river_length, road_length, armed_group, population, airport:military) %>% 
  mutate(coca_area = scale(log(1+coca_area))[,1],
         base_avg=scale(base_avg)[,1],
         hyd_avg=scale(hyd_avg)[,1],
         population=scale(population)[,1],
...
labs_2steps_2020_reg <- labs_2steps_2020 %>% 
  left_join(municipios_sf %>% as_tibble %>% select(id, area_km2), by="id") %>% 
  mutate(river_length = scale(river_length / area_km2)[,1],
         road_length = scale(road_length / area_km2)[,1]) %>% 
  select(-area_km2)
  ...
  # prices are excluded to compute lab_prob due to small number of obs.
PPI_lab_glm_2020 <- glm(PPI_lab~., family="binomial", data=labs_2steps_2020_reg %>% select(-id, -base_avg, -hyd_avg, -hyd_seizures, hyd_lab))
summary(PPI_lab_glm_2020)

hyd_lab_glm_2020 <- glm(hyd_lab~., family="binomial", data=labs_2steps_2020_reg %>% select(-id, -base_avg, -hyd_avg, -base_seizures, PPI_lab))
summary(hyd_lab_glm_2020)

regression_data_2020 <- labs_2steps_2020_reg %>% 
  mutate(PPI_lab_prob=PPI_lab_glm_2020$fitted.values,
         hyd_lab_prob=hyd_lab_glm_2020$fitted.values) %>% 
  select(-PPI_lab, -hyd_lab) %>% relocate(id, PPI_lab_prob, hyd_lab_prob)

hyd_avg_lm <- lm(hyd_avg~., regression_data_2020 %>% select(-id, -base_avg, -base_seizures, -PPI_lab_prob))
summary(hyd_avg_lm)
hyd_avg_pred <- predict(hyd_avg_lm, regression_data_2020 %>% select(-id, -base_avg, -base_seizures, -PPI_lab_prob))
regression_data_2020$hyd_avg <- ifelse(is.na(regression_data_2020$hyd_avg), hyd_avg_pred, regression_data_2020$hyd_avg)

base_avg_lm <- lm(base_avg~., regression_data_2020 %>% select(-id, -hyd_avg, -hyd_seizures, -hyd_lab_prob))
summary(base_avg_lm)
base_avg_pred <- predict(base_avg_lm, regression_data_2020 %>% select(-id, -hyd_avg, -hyd_seizures, -hyd_lab_prob))
regression_data_2020$base_avg <- ifelse(is.na(regression_data_2020$base_avg), base_avg_pred, regression_data_2020$base_avg)

# write.csv(regression_data_2020,
#           "Colombia Data/regression data all municipios lab_prob 2020.csv", row.names = F)
