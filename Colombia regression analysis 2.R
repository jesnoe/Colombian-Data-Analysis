...
base_source_glm <- glm(base_source~., family="binomial",  data=data_year %>% select(n_labs:population, base_source))
...
base_source_year %>% filter(posterior_glm < .5) %>% select(base_source, posterior_glm) %>% pull(base_source) %>% table
base_source_year %>% filter(posterior_glm >= .5) %>% select(base_source, posterior_glm) %>% pull(base_source) %>% table
base_source_year %>% filter(posterior_glm < .5 & base_source == 1) %>% select(base_source, posterior_glm) %>% arrange(posterior_glm)

names(data_year) <- gsub(" ", "_", names(data_year)) %>% stri_trans_general("Latin-ASCII") 
names(data_year)[14:36] <- c("Los_Rastrojos", "Clan_del_Golfo", "Las_Aguilas_Negras", "Los_Paisas",
...
base_source_rf <- randomForest(base_source~., data=data_year %>% select(n_labs:population, base_source), type="classification", ntree=50, mtry=20)
base_source_year$posterior_rf <- base_source_rf$votes[,2]
base_source_year %>% filter(posterior_rf < .5) %>% select(base_source, posterior_rf) %>% pull(base_source) %>% table
base_source_year %>% filter(posterior_rf >= .5) %>% select(base_source, posterior_rf) %>% pull(base_source) %>% table
base_source_year %>% filter(posterior_rf < .5 & base_source == 1) %>% select(base_source, posterior_rf) %>% arrange(posterior_rf)

base_source_year <- base_source_year %>% 
  mutate(y_glm=ifelse(posterior_glm < .5, 0, 1),
         y_rf=ifelse(posterior_rf < .5, 0, 1))

base_source_year %>%
  filter(y_glm != y_rf) %>% 
  select(base_source, posterior_glm, posterior_rf)
sum(base_source_year$base_source %>% as.numeric - 1) # 50
