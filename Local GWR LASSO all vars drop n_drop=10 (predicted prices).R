...
local_GWR_lasso <- function(type.measure_="default", interact_=F, scale_11_=F, weight_=NULL, dep_var, gwr_lasso_data_, method_, n_drop) {
...
# Local GWR lasso with log(seizures), log(coca_area) scaled data n_drop=10
...
cv_dev_min_mat_[,-1] <- NA
set.seed(5640)
start.time <- Sys.time()
local_GWR_coefs_lasso_hyd_dest_list <- local_GWR_lasso(dep_var = "hyd_destination", gwr_lasso_data_ = gwr_data, method_="var drop", n_drop = 10)
end.time <- Sys.time()
end.time - start.time # 1.11729 hours for hyd_destination model drop

local_GWR_coefs_lasso_hyd_dest_var_drop <- local_GWR_coefs_lasso_hyd_dest_list$lasso
write.csv(local_GWR_coefs_lasso_hyd_dest_list$cv_dev_min_mat, "Colombia Data/local GWR PML result predicted prices/local GWR lasso hyd_dest predicted price cv min dev var drop n_drop=10 (08-11-2025).csv", row.names = F)
save("local_GWR_coefs_lasso_hyd_dest_var_drop", file = "Colombia Data/local GWR PML result predicted prices/local GWR lasso hyd_dest predicted price var drop n_drop=10 (08-11-2025).RData")
