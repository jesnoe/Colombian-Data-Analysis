...
local_GWR_PML_1_year <- function(dep_var_, seed_model, reg_data_year, year_, price=F, weight_in=NULL) {
  ...
  # write.csv(local_GWR_coefs_PML_list$AUC_mat,
  #           sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out AUC all var drop log seizure coca scaled n_drop=10 weight %i-%i %i data CF (05-08-2026).csv",
  ...
  #      file = sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML %s leave-one-out all var drop log seizure coca scaled n_drop=10 weight %i-%i %i data CF (05-08-2026).RData",
...
local_gwr_PML_coef_map_by_AUC <- function(local_GWR_coefs_list, PML_best_bw_tbl_, criteria, dep_var, alpha=0.1, n_drop, date_, year_, indep_vars_, price) {
  ...
  write.csv(coef_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs %s violence_all left-right all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  
  write.csv(pval_table,
            sprintf("Colombia Data/local GWR PML result predicted prices/local GWR PML p-value %s violence_all left-right all var drop by AUC n_drop=%i %i data %s CF (%s).csv", dep_var, n_drop, year_, title_for_price, date_),
            row.names = F)
  ...
  for (i in c(2, 4:length(coef_table))) {
    ...
    ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/coef maps/%s (%i)/local GWR PML coef by AUC violence_all left-right %s %s all var drop n_drop=%i %i data %s CF (%s).png",
...
variable_influence_map_CF <- function(variable_influence_CF_year, selected_id=NULL, dep_var, n_drop, year_) {
  ...
  for (i in c(2:length(variable_influence_CF_year))) {
    ...
    if (is.null(selected_id)) {
      ...
      ggsave(sprintf("Colombia Data/local GWR PML result predicted prices/VI maps CF/VI maps CF (y=1)/%s (%i)/local GWR PML variable influence %s %s all var drop %i %i data %s CF.png",
