...
indep_vars <- c("coca_area", "seizures", "river_length", "road_length", "population", "airport", "ferry", "police", "military", "lab_prob", "left_wing", "right_paramilitary")
...
indep_vars_ <- names(PML_gwr_coefs_AUC_lab_prob_1617)[-(1:2)]
}
...
start_time <- Sys.time()
for (j in 1:nrow(PML_gwr_coefs_AUC_lab_prob_1617)) {
  id_j <- PML_gwr_coefs_AUC_lab_prob_1617$id[j]
...
# y_tbl <- bind_rows(regression_data_lab_prob_2016 %>% select(id, hyd_destination) %>% rename(y = hyd_destination),
#                    regression_data_lab_prob_2017 %>% select(id, hyd_destination) %>% rename(y = hyd_destination))
# y_tbl$year <- c(rep(2016, nrow(regression_data_lab_prob_2016)), rep(2017, nrow(regression_data_lab_prob_2017)))
...
sample(PML_gwr_coefs_AUC_lab_prob_1617$id, 5)
...
PML_gwr_coefs_AUC_lab_prob_1617 %>% filter(id %in% c(5002, 91263, 41615, 52258))
...
library(distances)
{
  ...
  base_gwr_data <- read.csv("Colombia Data/base gwr data.csv") %>% as_tibble
  hyd_gwr_data <- read.csv("Colombia Data/hyd gwr data.csv") %>% as_tibble
  PML_gwr_coefs_AUC_lab_prob_1617 <- read.csv("Colombia Data/local GWR PML result predicted prices/local GWR PML coefs hyd_destination leave-one-out PML_log_seizure_coca_bw_F1 all var drop 10 (12-09-2025).csv") %>% as_tibble
...
coca_area_list_hyd_dest <- area_of_interest_list(PML_gwr_coefs_AUC_lab_prob_1617, PML_gwr_p_vals_AUC_var_drop_log_seizure_coca_10_loo_hyd_dest, coca_area, 1)

river_length_list_hyd_dest <- area_of_interest_list(PML_gwr_coefs_AUC_lab_prob_1617, PML_gwr_p_vals_AUC_var_drop_log_seizure_coca_10_loo_hyd_dest, river_length, 0)
...
armed_group_list_hyd_dest <- area_of_interest_list(PML_gwr_coefs_AUC_lab_prob_1617, PML_gwr_p_vals_AUC_var_drop_log_seizure_coca_10_loo_hyd_dest, armed_group, 2)
police_list_hyd_dest <- area_of_interest_list(PML_gwr_coefs_AUC_lab_prob_1617, PML_gwr_p_vals_AUC_var_drop_log_seizure_coca_10_loo_hyd_dest, police, -Inf)
